...
// Define selectors for completed and not completed items
const selectors = [':host([completed="true"]) li', ':host([completed="false"]) li'];

for (const selector of selectors) {
    additionalStyleSheet.insertRule(`
        ${selector} {
            border-bottom-color: var(--complex-border-bottom-color-${selector.includes("true") ? "completed" : "not-completed"});
            background-color: var(--complex-background-color-${selector.includes("true") ? "completed" : "not-completed"});
        }
    `);

    additionalStyleSheet.insertRule(`
        ${selector} > div > label {
            color: var(--complex-color-${selector.includes("true") ? "completed" : "not-completed"});
        }
    `);
}
